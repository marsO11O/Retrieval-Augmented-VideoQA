...
def main(args):
    ...
    for index_file in sorted(args.index_dir.glob("*.index")):
        ...
        for qid in vid_qids:
            ...
            top_rerank_cap_ids = [r["cap_id"] for r in results[:args.topn]]

            # Top-N TXT
            top_txt_cap_ids = [h["meta"].get("id", -1) for h in txt_hits[:args.topn]]

            # Top-N IMG
            top_img_cap_ids = [h["meta"].get("clip_idx", -1) for h in img_hits[:args.topn]]
            ...
            # check
            if any(start <= cap_id <= end for cap_id in top_rerank_cap_ids):
                correct_rerank += 1
            if any(start <= cap_id <= end for cap_id in top_txt_cap_ids):
                correct_txt += 1
            if any(start <= cap_id <= end for cap_id in top_img_cap_ids):
                correct_img += 1
    ...
    print(f"Correct in Top-{args.topn} (TXT+IMG Rerank): {correct_rerank} (Accuracy={correct_rerank/total:.4f})")
    print(f"Correct in Top-{args.topn} (TXT Only): {correct_txt} (Accuracy={correct_txt/total:.4f})")
    print(f"Correct in Top-{args.topn} (IMG Only): {correct_img} (Accuracy={correct_img/total:.4f})")
